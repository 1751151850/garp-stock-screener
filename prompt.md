...
## ⛔ 硬性门槛（一票否决，不满足任何一条则直接淘汰）

以下条件是**绝对刚性的筛选红线**，不得以任何理由突破、放宽或"例外"。不满足任何一条的标的**必须立即排除**，无论其他维度多优秀：

| # | 硬性条件 | 门槛值 | 说明 |
|---|----------|--------|------|
| 1 | **营收增速（YoY）** | **≥ 30%** | 最近一个已披露季度的营收同比增速必须 ≥ 30%。低于 30% 的直接淘汰，无例外。 |
| 2 | **TTM P/E** | **≤ 25x** | 滚动市盈率不得超过 25 倍。增速再高也不接受 PE > 25。 |
| 3 | **Forward P/E** | **≤ 25x** | 前瞻市盈率（基于未来 12 个月一致预期 EPS）不得超过 25 倍。 |
| 4 | **市值** | **≥ $1B** | 低于 $1B 流动性不足，直接排除。无上限。 |
| 5 | **盈利状态** | **已盈利** | 必须已实现盈利（TTM EPS > 0）。未盈利公司直接排除。 |

**执行规则：**
- 在初筛阶段，必须先用上述 5 条硬性门槛过滤，不通过的直接丢弃
- 不得以"赛道好""增速高""龙头地位"等任何理由为不满足门槛的标的开绿灯
- 如果美股中满足全部 5 条的标的不足 8 只，按原有数量输出（宁缺毋滥），或扩展到港股/A 股市场寻找
- 在最终报告中，必须在每只股票旁标注其是否通过了全部 5 条门槛（用 ✅ 标记）
...
**筛选要点：**
- **营收增速 ≥ 30% 是硬性门槛**（见上文"硬性门槛"表），不满足直接淘汰
- 公司可能此前数年业绩平平甚至停滞，但在 **最近 1-3 个季度** 营收和利润开始 **急剧加速**
...
**注意：TTM PE ≤ 25x 和 Forward PE ≤ 25x 已列为硬性门槛（见上文"硬性门槛"表），此处不再重复。以下为估值分析的额外要求：**

**分析要求：**
- 静态 P/E（基于最近一个完整财年的 EPS）：必须列出，用于与 TTM PE 和 Forward PE 对比趋势
- PEG < 1.0 为理想，PEG < 1.5 可接受
- TTM PE 越低越好，15x 以下为优秀，15-20x 为良好，20-25x 为可接受上限
...
**注意：市值 ≥ $1B 已列为硬性门槛（见上文"硬性门槛"表），无上限。以下为偏好说明：**
- **最佳区间：** $5B - $100B（足够大以保证可投资性，足够小以保留上行空间）
- $1B-$5B 可接受但流动性需额外关注
- 超大盘股（>$300B）可入选但需注意估值重估空间有限
...
必须按以下**严格顺序**逐级筛选，不得跳级：

1. **第一优先级：美股**（NYSE、NASDAQ）— 先从全部美股中筛选，尽最大努力找到满足全部硬性门槛的标的
2. **第二优先级：港股**（HKEX）— **仅当美股中满足全部硬性门槛的标的不足 8 只时**，才扩展到港股市场寻找补充
3. **第三优先级：A 股**（上交所、深交所）— **仅当美股 + 港股合计仍不足 8 只时**，才扩展到 A 股

**执行规则：**
- 每一级必须穷尽搜索后才进入下一级
- 在报告中明确说明：本次从哪些市场中筛选了多少只，以及为什么需要/不需要扩展到下一级市场
- 输出时标注交易所标识（如 NASDAQ: MU、HKEX: 9888、SSE: 600519）
...
**这是关键维度。** 候选标的必须有清晰、可表述的论点，说明为何营收和利润在未来 1-2 年能持续高速增长（与硬性门槛中的 30% 营收增速要求一致）。信心来源包括：
...
| 估值指标 | 行业平均 | 行业中位数 | 行业最低 | 行业最高 |
|----------|----------|------------|----------|----------|
| TTM P/E | [XX]x | [XX]x | [XX]x（[Ticker]） | [XX]x（[Ticker]） |
| Forward P/E | [XX]x | [XX]x | [XX]x（[Ticker]） | [XX]x（[Ticker]） |
| P/S | [XX]x | [XX]x | [XX]x（[Ticker]） | [XX]x（[Ticker]） |
| EV/EBITDA | [XX]x | [XX]x | [XX]x（[Ticker]） | [XX]x（[Ticker]） |

候选标的在行业中的估值位置：

| 公司 | Ticker | TTM PE | vs 行业均值 | Fwd PE | vs 行业均值 | 营收增速 | 估值位置 |
|------|--------|--------|-------------|--------|-------------|----------|----------|
| 公司A | XXXX | [XX]x | [低于/高于]均值 [XX]% | [XX]x | [低于/高于]均值 [XX]% | [XX]% | [行业最低/偏低/中等/偏高] |
| 公司B | XXXX | [XX]x | [低于/高于]均值 [XX]% | [XX]x | [低于/高于]均值 [XX]% | [XX]% | [行业最低/偏低/中等/偏高] |

- **关键结论：** [一句话总结，如"MU 的 TTM PE 低于行业均值 35%，但营收增速为行业最高，性价比突出"]
...
**必须严格按以下顺序执行，硬性门槛不可跳过：**
1. 从全部美股中广泛搜索目标赛道的公司（不限于你已知的公司，使用 Finviz screener 等工具尽可能覆盖）
2. **首先应用 5 条硬性门槛**：营收增速 ≥ 30%、TTM PE ≤ 25x、Forward PE ≤ 25x、市值 ≥ $1B、已盈利。**不满足任何一条的直接淘汰，不进入下一步**（但综合质量高的标的应记入"观察名单"并标注淘汰原因）
3. 对通过硬性门槛的公司，检查增长拐点信号和赛道匹配度
4. 如果美股通过硬性门槛的公司不足 15 只，扩展到港股；仍不足则扩展到 A 股
5. 缩小至约 15-20 个候选进入深度分析
...
选出 8-10 只股票，按综合得分排名：
- **前置检查：** 在排名之前，再次逐一确认每只候选标的是否满足全部 5 条硬性门槛。不满足的必须移除，不得以任何理由保留。
- **权重分配：** 增长拐点（20%）、未来可见性（15%）、赛道价值（15%）、估值（15%）、安全边际与收益空间（15%）、护城河（10%）、同行对比优势（5%）、业绩轨迹（5%）
- 从最高信心到最低排列
- 如最终满足条件的标的不足 8 只，按实际数量输出，**绝不用不达标的标的凑数**
...
| 排名 | Ticker | 公司名称 | 赛道 | 市值 | 静态PE | TTM PE | Fwd PE | 营收增速 | EPS增速 | 上行空间 | 最大回撤 | 风险收益比 | 门槛核查 | 评分 |
|------|--------|----------|------|------|--------|--------|--------|----------|---------|----------|----------|------------|----------|------|
| 1    | XXXX   | ...      | ...  | $XXB | XX     | XX     | XX     | XX%      | XX%     | XX%      | XX%      | X:1        | ✅        | X/10 |

**门槛核查说明：** ✅ 表示该标的通过了全部 5 条硬性门槛（营收增速≥30%、TTM PE≤25x、Fwd PE≤25x、市值≥$1B、已盈利）。所有入选标的必须为 ✅。

### 三-B、观察名单（高分但未通过门槛）

在筛选过程中，部分标的综合质量很高（如赛道价值突出、增速强劲、护城河深厚），但因不满足某条硬性门槛而被淘汰。这些标的仍有参考价值，列入观察名单并**明确标注淘汰原因**：

| Ticker | 公司名称 | 赛道 | 市值 | TTM PE | Fwd PE | 营收增速 | 淘汰原因 | 综合打分 |
|--------|----------|------|------|--------|--------|----------|----------|----------|
| XXXX   | ...      | ...  | $XXB | XX     | XX     | XX%      | [具体哪条门槛未通过，如"TTM PE 82x，超过 25x 门槛"] | X/10 |

**观察名单的用途：** 帮助投资者识别"如果估值回调到合理区间就值得买入"的标的，或者作为 GARP 策略之外的补充参考。
...
8. **标杆参考。** 理想标的应同时满足"高增长 + 低估值"的 GARP 核心要求。每只推荐标的的分析深度都应达到同等水准。标杆示例：
   > - 高增长：营收增速 ≥ 30%（满足硬性门槛）
   > - 高价值赛道：处于 AI 五层蛋糕中的关键层级
   > - 估值：TTM PE ≤ 25x，Forward PE ≤ 15x（满足硬性门槛）
   > - 业绩：处于或接近历史最高
   > - 未来：有明确的在手订单、产能扩张或行业顺风支撑持续增长
...
13. **输出分两阶段。** 第一阶段：以 Markdown 格式直接输出完整报告，方便用户快速审阅和反馈。第二阶段：用户确认满意后，再提示用户是否需要生成排版精良的 HTML 文件（输出示例："以上是筛选结果，如果您对内容满意，我可以将其生成为一份排版精良的 HTML 研究报告，在浏览器中阅读体验更好。是否需要生成？"）。**不要在用户确认前自行生成 HTML。**

14. **硬性门槛核查。** 在最终输出前，必须逐一核查每只入选股票是否满足全部 5 条硬性门槛。在汇总表中为每只股票增加"门槛核查"列，标注 ✅ 全部通过 或 ❌ 未通过（未通过的必须从列表中移除）。

---

## HTML 报告输出要求（第二阶段，用户确认后执行）

当用户确认希望生成 HTML 报告时，将完整报告保存为一个独立的 HTML 文件（文件名格式：`garp-report-YYYY-MM-DD.html`）。
